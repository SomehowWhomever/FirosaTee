  ...
  useEffect(() => {
    async function loadShopData() {
      try {
        ...
        const filtered = data.filter((p: any) => p.category === 'occasions');
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-[#2D3A3A] tracking-tight">
            Holiday & <span className="text-[#008080]">Occasions</span>
          </h1>
          <p className="mt-2 text-neutral-500 font-medium max-w-xl">
            Limited-run, custom drops designed specifically for your life events, holidays, and milestones.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.length === 0 ? (
              <div className="col-span-full text-center py-12 text-neutral-400 font-medium">No custom event items found.</div>
            ) : (
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-[300] bg-[#F4F7F6] flex flex-col items-center justify-center">
          ...
          <nav className="flex flex-col gap-6 text-center">
            ...
            <Link href="/occasions" className="text-3xl font-bold text-[#008080]">Occasions</Link>
            <Link href="/mens" className="text-3xl font-bold text-[#2D3A3A] hover:text-[#008080]">Mens</Link>
            <Link href="/womens" className="text-3xl font-bold text-[#2D3A3A] hover:text-[#008080]">Women & Children</Link>
